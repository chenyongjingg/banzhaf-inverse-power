"""Confirm D=4 uniform construction for ALL n ≡ 4 mod 7."""
...
print("=== D=4, f=6 for ALL n ≡ 4 mod 7 ===")
works=0; fails=[]
for n in range(18, 200):
    if n%7!=4: continue
    found=None
    for q in range(7):
        r=construct(n,4,6,q)
        if r is not None and r[4]: found=q; break
    if found is not None:
        works+=1
    else:
        fails.append(n)
print(f"D=4: {works} work, fails: {fails}")
print()
print("=== n=18 explicit construction (D=4, f=6, q=1) ===")
r=construct(18,4,6,1)
if r:
    c,s,G,F,ok,mono=r
    print(f"c={c} s={s} |G|={len(G)} |F|={len(F)} OK={ok} monotone={mono}")
    print("F (triples):", F)
